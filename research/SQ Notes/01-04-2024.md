...
Yes, silk threads are used in the process of making silk fabric. The process of silk making involves several steps, including rearing silkworms, harvesting cocoons, and spinning silk fibers into yarn 1. The yarn is then woven into fabric using a loom 2.

To spin silk fibers into yarn, the spinner first takes a few strands of silk from the mass on the distaff and hooks it to a spindle 1. The spindle is then rotated, and the spinner uses their fingers to control the thickness of the yarn 1. The spindle is used to twist the silk fibers into long threads, which can then be wound onto a bobbin or spindle 1. Once the silk fibers have been spun into yarn, they can be woven into fabric using a loom 2.
